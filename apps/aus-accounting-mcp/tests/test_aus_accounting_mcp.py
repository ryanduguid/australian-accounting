...
def test_client_snippets_use_uvx_from_pypi() -> None:
    ...
    assert "payday-super-checker==0.1.6" in pyproject
...
def test_release_metadata_matches_the_candidate_version() -> None:
    ...
    assert project["version"] == "0.2.4"
    assert re.search(r"(?m)^version: 0\.2\.4$", citation)
    assert "date-released:" not in citation
    assert re.findall(r"(?m)^# (v\S+)$", release_notes)[0] == "v0.2.4"
    assert "ato-benchmark-compare` 0.1.8" in release_notes
    assert "payday-super-checker` 0.1.6" in release_notes
    assert "div7a-loan-review` 0.1.4" in release_notes
    assert published_version == server["packages"][0]["version"] == "0.2.4"
...
def test_readme_has_stable_proof_anchor_and_mapping() -> None:
    ...
    for text in (
        ...
        "https://pypi.org/project/aus-accounting-mcp/0.2.4/",
        "https://registry.modelcontextprotocol.io/v0.1/servers/io.github.ryanduguid%2Faus-accounting/versions/0.2.4",
...
def test_readme_links_to_release_records() -> None:
    ...
    assert (
        f"[v0.2.4 release record]({CANONICAL_REPOSITORY}/releases/tag/"
        "aus-accounting-mcp/v0.2.4)"
        in readme
    )
