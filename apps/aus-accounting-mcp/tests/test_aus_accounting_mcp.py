...
def test_release_metadata_matches_the_candidate_version() -> None:
    ...
    assert published_version == server["packages"][0]["version"] == "0.2.9"
...
def test_readme_has_stable_proof_anchor_and_mapping() -> None:
    ...
    for text in (
        ...
        "https://pypi.org/project/aus-accounting-mcp/0.2.9/",
        "https://registry.modelcontextprotocol.io/v0.1/servers/io.github.ryanduguid%2Faus-accounting/versions/0.2.9",
...
def test_readme_links_to_release_records() -> None:
    ...
    assert (
        f"[v0.2.9 release record]({CANONICAL_REPOSITORY}/releases/tag/"
        "aus-accounting-mcp/v0.2.9)"
        in readme
    )
