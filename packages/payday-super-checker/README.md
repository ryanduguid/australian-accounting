...
That stop is the tool working as designed. LCR 2026/1 applies 1 to 28 July 2026
...
Both blocks are abridged in the same way as the 2 above: the real runs also
...
This uses the existing checker and practitioner checklist to write 4 files
...
AT_RISK`. A caveat may name an intermediate outcome where more than 2 remain
...
- **Assessment rounding** is not reproduced. The tool displays report components to cents with `ROUND_HALF_UP`; TAA 1953 s 16B applies a 5-cent down-round only to the Commissioner's final assessed SG charge. Exposure figures are experimental estimates.
...
Those are the things the tool never attempts. Separately, [LIMITATIONS.md](LIMITATIONS.md) records the cases where it does produce a figure that you should not read at face value, naming what stays correct in each.
