...
4. Create an annotated tag on the current remote `main` commit and push that tag:
   ...
   ```bash
   git tag -a payday-super-checker/v0.1.7 \
     -m "payday-super-checker v0.1.7"
   git push origin refs/tags/payday-super-checker/v0.1.7
   ```
...
```bash
tag=payday-super-checker/v0.1.7
repo=ryanduguid/australian-accounting
release_commit=$(git ls-remote "https://github.com/$repo.git" \
  "refs/tags/$tag^{}" | cut -f1)
test -n "$release_commit"
gh release download "$tag" -R "$repo" --dir payday-super-checker-v0.1.7
cd payday-super-checker-v0.1.7
sha256sum --check SHA256SUMS
gh release verify "$tag" -R "$repo"
gh release verify-asset "$tag" payday_super_checker-0.1.7-py3-none-any.whl \
  -R "$repo"
gh attestation verify payday_super_checker-0.1.7-py3-none-any.whl \
  -R "$repo" \
