...
The current pass read the Federal Register, the ATO legal database, direct ATO rate guidance and all 8 official jurisdiction holiday sources. F2026L00784, SGAR regulations 11 to 13D, the $270,830 maximum contribution base and LCR 2026/1 to 3 are verified directly. LCR 2026/D1 remains draft pending appeal, so qualifying-earnings classification remains outside the runtime. Full source links, runtime comparisons and residuals are in `primary-source-review-2026-08-15.md`.

LCR 2026/1, LCR 2026/2 and LCR 2026/3 were issued on 5 August 2026;
LCR 2026/D1 is the only one of the original 4 drafts that remains draft.

- *Treasury Laws Amendment (Payday Superannuation) Act 2025* (No 57 of 2025), assent 6 November 2025; SGC Amendment Act 2025 (No 58 of 2025); *Payday Superannuation Regulations 2026* (F2026L00133). Applies to QE days from 1 July 2026.
...
- Business day (s 6(1)): not Sat/Sun, not a public holiday applying to the **whole of any** State/ACT/NT. One national calendar. Regional holidays (for example, Royal Queensland Show) are business days.
...
- SGC on lateness (s 16B(2)): final shortfall + notional earnings component + administrative uplift (+ choice loading, out of scope). NEC: daily compounding at GIC rate on base shortfall, from the day **after** the last on-time day, until final shortfall nil or day before assessment (s 19A; final LCR 2026/3). Uplift: 60% of (shortfalls + NEC), reduced 20pp clean-history (reg 13C, transitional lookback from 1 July 2026) and 40/35/30/15pp by voluntary-disclosure timing (reg 13D). Floor 0%.
...
- `atomic_io.py` - generated-output boundary for all 3 paths: an explicit `.csv` destination for contribution/report files or `.md` for the practitioner pack, staged in that destination's own directory and moved into place with `os.replace`, so an existing symlink at the destination is replaced rather than followed and a failed write leaves no partial file at the chosen name.
...
Pre-1 July 2026 QE days: hard error (old quarterly law, out of scope).

## Data files

- `data/business_days.json` - official-source-complete non-business days from 1 July 2026 through 2027-08-31, plus reference dates after the horizon. Generated by `tools/generate_calendar.py` (dev-time only, python-holidays pinned), checked against all 8 official jurisdiction sources and shipped with those URLs/check date. Regional, part-day and locally substitutable dates are excluded, including WA King's Birthday and Melbourne Cup Day. Business Victoria still lists the exact 2027 grand-final holiday as subject to the AFL schedule, so later dates sit beyond the completeness horizon. Unconfirmed future grand-final dates are retained as provisional reference entries and do not extend deadlines.
...
- ATO example: first QE day 9 July 2026 -> extended due 7 August 2026.
- LCR 2026/3 example: QE day 8 June 2027, usual period ends 18 June 2027, NEC starts 19 June 2027.
- Ekka 12 August 2026 counted as business day; WA Day / whole-of-state holidays not.
